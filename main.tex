...
% Equal-contribution authors take one of two annotations:
%   1=equal           → name + \textsuperscript{*}, no bold
%   2=highlightequal  → \textbf{name} + \textsuperscript{*}
% (biblatex's +an field is one-annotation-per-position; we combine them
%  into a single value rather than stacking comma-separated annotations.)
\renewcommand*{\mkbibnamefamily}[1]{%
  \ifitemannotation{highlight}{\textbf{#1}}{%
    \ifitemannotation{highlightequal}{\textbf{#1}\textsuperscript{*}}{%
      \ifitemannotation{equal}{#1\textsuperscript{*}}{#1}}}}
\renewcommand*{\mkbibnamegiven}[1]{%
  \ifitemannotation{highlight}{\textbf{#1}}{%
    \ifitemannotation{highlightequal}{\textbf{#1}}{#1}}}
\renewcommand*{\mkbibnameprefix}[1]{%
  \ifitemannotation{highlight}{\textbf{#1}}{%
    \ifitemannotation{highlightequal}{\textbf{#1}}{#1}}}

% Optional citation metrics — populated by scripts/fetch_citations.py
% (Semantic Scholar with OpenAlex fallback). If metrics.tex is missing
% (fresh clone, never ran the script), define empty fallbacks so the CV
% still builds. metrics.tex uses \newcommand, so we can't \providecommand
% first or the \newcommand will clash on "already defined".
\IfFileExists{metrics.tex}{\input{metrics.tex}}{%
  \newcommand{\citationsTotal}{}%
  \newcommand{\citationsHIndex}{}%
  \newcommand{\citationsAsOf}{}%
}
...
{\footnotesize\itshape\color{meta}Multimodal foundation models for tabular data}\\[3pt]
...
\role{Centrum Wiskunde \& Informatica \& University of Amsterdam}{Aug 2026 \textendash{} Present}
PhD in Computer Science. \textbf{Advised by \href{https://madelonhulsebos.github.io/}{Madelon Hulsebos} (\href{https://trl-lab.github.io/}{TRLLab}, CWI) and \href{https://jwvdm.github.io/}{Jan-Willem van de Meent} (AMLab, UvA); \href{https://ellis.eu/phd-postdoc}{ELLIS PhD Program} international co-advisor TBD.} Focus: multimodal tabular foundation models.

\role{Tufts University, Medford, MA}{Sep 2024 \textendash{} May 2026}
M.S.\ in Computer Science. GPA: 3.96\,/\,4.00. \href{https://loevlie.github.io/cv/Final_Tufts_Transcript.pdf}{Transcript}.
...
\role{Centrum Wiskunde \& Informatica \bdash{} \href{https://trl-lab.github.io/}{TRLLab}}{Aug 2026 \textendash{} Present}
\sub{PhD Researcher with \href{https://madelonhulsebos.github.io/}{Madelon Hulsebos}; co-advised by \href{https://jwvdm.github.io/}{Jan-Willem van de Meent} (AMLab, UvA)}
\begin{itemize}
  \item Developing multimodal tabular foundation models.
\end{itemize}

\role{Tufts University \bdash{} Hughes Lab}{Aug 2024 \textendash{} Aug 2026}
\sub{Graduate Researcher with Prof.\ Michael Hughes}
\begin{itemize}
  \item Co-led (equal contribution) the \acro{CHIL} 2026 multi-dataset benchmark of multiple instance learning (\acro{MIL}) for 3D neuroimage classification across 7 \acro{MRI}/\acro{CT} datasets and 9 tasks; showed a simple mean-pooling \acro{MIL} baseline matches or outperforms recent attention-based \acro{MIL} and 3D \acro{CNN} methods on 4 of 6 moderate-sized tasks while being \textbf{25$\times$ faster to train}.
  \item Contributed to the \acro{ML4H} 2025 study on a \emph{generalization gap} between correlated and non-correlated \acro{MIL} architectures: implemented \acro{SmAP} (the newest NeurIPS \acro{MIL} method) into the lab codebase, reduced a training-example bottleneck from \emph{quadratic to linear scaling} to unlock experiments on a substantially larger semi-synthetic dataset, and helped design the synthetic benchmark with a Bayes-optimal upper bound.
  \item Designed convolutional and transformer-based classification pipelines on 3D \acro{MRI}/\acro{CT} for early indicators of dementia and stroke; trained on Tufts \acro{HPC}, parallelizing experiments across many \acro{GPU}s via \acro{SLURM} job arrays.
  \item Developed a regularization method to encourage interpretable attention scores, and methods for supervised learning under noisy labels.
\end{itemize}

\role{Tufts University \bdash{} Sinapov Lab}{Jan 2025 \textendash{} Aug 2026}
...
\begin{itemize}
  ...
  \item Investigated why vision-language models (Qwen3-\acro{VL}) and open-vocabulary segmentation models (\acro{SAM}\,3) fail at object counting in dense scenes; designing tool-use and trajectory-based methods drawing on cognitive-science accounts of human counting.
\end{itemize}
...
\begin{itemize}
  ...
  \item Ensured fair evaluation of \acro{ML} architectures for material-property prediction; co-author on \emph{Salem et al., JPCC 2023}.
...
\begin{itemize}
  \item \textbf{Models:} fine-tuned HoVer-NeXt on our in-house prostate epithelial-nuclei annotations for nuclear instance segmentation, and implemented \href{https://github.com/TissueImageAnalytics/cerberus}{Cerberus} for gland segmentation on gigapixel prostate-cancer slides.
  \item \textbf{Systems:} composing both models into an end-to-end pipeline that extracts quantitative tissue features as an objective alternative to subjective Gleason scoring for prostate-cancer diagnosis; integrates with clinical workflows for downstream evaluation.
...
\role{KEF Robotics, Pittsburgh, PA}{Jan 2024 \textendash{} Aug 2024}
\sub{Senior Computer Vision \& \acro{ML} Engineer}
\begin{itemize}
  \item \textbf{Leadership:} sole \acro{ML} engineer leading a team of five on a one-year, \$500K project; owned task breakdown, budgeting, and applied-research direction.
  \item \textbf{Systems:} led the development of on-device object detection with 3D localization, monocular depth estimation, and 3D map generation from monocular drone imagery; deployed on flight hardware via TensorRT. Two in-person demos secured the next round of funding.
\end{itemize}

\role{KEF Robotics, Pittsburgh, PA}{Jan 2023 \textendash{} Dec 2023}
\sub{Computer Vision Engineer}
\begin{itemize}
  \item \textbf{Research:} designed a three-stage multimodal semantic-segmentation pipeline for tethered-drone power-line detection under severe data constraints \bdash{} (1) Mask2Former fine-tuned from \acro{COCO} on drone simulation data, (2) progressive sim-to-real domain adaptation using limited labeled real-world \acro{RGB} imagery, (3) automated cross-modal label transfer via geometric correspondence to synthesize \acro{IR} training data from \acro{RGB} predictions, removing the need for manual \acro{IR} annotation.
  \item \textbf{Systems:} optimized the segmentation architecture for a \textbf{45\% inference-speed gain} at $<\!1\%$ accuracy cost; work featured in invited talk at \emph{XChangeIdeas Pittsburgh} (1 of 2 \acro{KEF} presenters; 100+ industry attendees).
\end{itemize}
...
\begin{itemize}
  \item \href{https://github.com/huggingface/transformers}{\monofam huggingface/transformers} \,\textemdash\, contributor; merged fix restoring \monofam{torch.compile} compatibility for Mask2Former and OneFormer
  \item \href{https://github.com/Franblueee/torchmil}{\monofam Franblueee/torchmil} \,\textemdash\, contributor; added the ML4H 2025 synthetic dataset and Bayes-optimal upper bound
  ...
  \item \href{https://github.com/loevlie/gpt4readability}{\monofam loevlie/gpt4readability} \,\textemdash\, author; README generation and code-improvement suggestions for any code repository, powered by OpenAI or a local Llama model
...
\begin{publist}
\item \href{https://drive.google.com/file/d/18nFiw33ZuQlHw3z-uM-b8ROkWn8_ZpR2/view?usp=sharing}{\textit{A Multi-Dataset Benchmark of Multiple Instance Learning for 3D Neuroimage Classification}}. Poster, CHIL 2026, Seattle, WA. \textit{June 2026.}
...
\noindent\textbf{Languages} \quad Python $\cdot$ C++ $\cdot$ Java $\cdot$ JavaScript $\cdot$ MATLAB \\
\textbf{\acro{ML} / AI} \quad PyTorch $\cdot$ JAX $\cdot$ Hugging Face (Transformers, Datasets, \acro{TRL}, \acro{PEFT}) $\cdot$ multimodal learning $\cdot$ \acro{RL} post-training $\cdot$ multiple instance learning $\cdot$ computer vision \\
\textbf{Engineering} \quad \acro{CUDA} $\cdot$ TensorRT $\cdot$ \acro{SLURM} / \acro{HPC} $\cdot$ Docker $\cdot$ \acro{AWS}
