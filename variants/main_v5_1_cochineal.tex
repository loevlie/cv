%---------------------------------------------------------------------------
% Dennis Johan Loevlie -- Academic CV (v5.1 — Cochineal body)
% Body: Cochineal (Caslon revival, beautiful italic, real small caps)
% Headers/dates: IBM Plex Sans (sans accent rhythm)
% URLs: IBM Plex Mono
% Engine: pdflatex (Cochineal type1 ships small caps)
%---------------------------------------------------------------------------
...
% --- Typography ----------------------------------------------------------
% Cochineal: scale slightly up to match Plex Sans optical x-height.
% osf+p = old-style proportional figures by default in body.
\usepackage[scale=1.02,osf,p]{cochineal}
...
% v5.1: Cochineal package option `osf` already sets old-style figures
% as the body default; no \figureversion call needed at begin-document.
...
% --- Section / subsection ------------------------------------------------
% v5.1: Cochineal ships real small caps. Section headers in serif SC,
% letter-spaced. Drops the Plex Sans accent in section heads in favor
% of the genuine SC texture — peak Bringhurst register.
...
% Date column gets lining tabular figures via fontaxes' figureversion
% (still works since Cochineal loads fontaxes for font axes control).
...
% v5.1: Venue + acro use REAL small caps from Cochineal. The textsc
% input is lowercase so we get proper SC glyphs (textsc on uppercase
% input falls back to caps in some implementations).
