%---------------------------------------------------------------------------
% Dennis Johan Loevlie -- Academic CV (v4 — LuaLaTeX)
% Same content as main.tex, but built with lualatex so plex-serif/sans/mono
% auto-load via fontspec from OTF, unlocking REAL small caps (\textsc{}),
% finer kerning, and direct OpenType feature control.
%
% Build: lualatex main_v4_lualatex.tex (run twice for lastpage refs)
%---------------------------------------------------------------------------
\documentclass[10pt,letterpaper]{article}

% LuaLaTeX is native Unicode; T1 fontenc and utf8 inputenc are pdftex-only.
% iftex lets the same source compile under either engine if needed later.
\usepackage{iftex}
\ifPDFTeX
  \usepackage{cmap}
...
% LuaTeX does kerning natively in the font shaper — microtype's `kerning`
% option is pdftex-only and would error. Drop it; protrusion/expansion/
% tracking are still active.
\usepackage[protrusion=true,expansion=true,tracking=true,final]{microtype}

% Old-style proportional figures by default in body text. Under pdflatex,
% plex-serif loads fontaxes (so \figureversion works); under LuaLaTeX it
% uses fontspec instead (use \addfontfeatures). Provide a single
% \osfigures macro that works under either engine.
\ifPDFTeX
  \newcommand{\osfiguresnow}{\figureversion{osf}}
  \newcommand{\lffiguresnow}{\figureversion{lf}}
\else
  \newcommand{\osfiguresnow}{\addfontfeatures{Numbers=OldStyle}}
  \newcommand{\lffiguresnow}{\addfontfeatures{Numbers=Lining}}
\fi
\AtBeginDocument{\osfiguresnow}
...
% --- Section / subsection ------------------------------------------------
% v4 LuaLaTeX note: IBM Plex Serif / Plex Sans both lack the `smcp`
% OpenType feature — small caps were never drawn for the family. So we
% keep the same spaced-uppercase workaround as v2/v3. The engine win
% lives elsewhere (better kerning, native Unicode, OpenType feature
% access — see the slashed-zero/stylistic-set hooks below).
...
\newcommand{\datestamp}[1]{{\sansfam\footnotesize\color{meta}\lffiguresnow #1}}
...
% Venue + acronym treatment — same workaround as v2/v3 (Plex has no SC).
